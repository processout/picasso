...
export abstract class Chart {
  ...
  constructor(el: string, opt: Options) {
    ...
    opt.marginRight =
      opt.marginRight != null ? opt.marginRight : opt.xAxisMargin + 40;
    opt.marginBottom =
      opt.marginBottom != null ? opt.marginBottom : opt.yAxisMargin + 30;
    opt.marginLeft =
      opt.marginLeft != null ? opt.marginLeft : opt.xAxisMargin + 40;
  ...
  protected init(el: string, opt: Options): void {
    ...
    // Compute size
    this.width =
      +this.root.node().getBoundingClientRect().width -
      opt.marginLeft -
      opt.marginRight;
    this.height =
      +this.root.node().getBoundingClientRect().height -
      opt.marginTop -
      opt.marginBottom;
    // Set padding
    this.svg = this.root
      .append("g")
      .attr("transform", this.translate(opt.marginLeft, opt.marginTop));
  }
